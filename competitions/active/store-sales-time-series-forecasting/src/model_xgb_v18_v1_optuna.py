#!/usr/bin/env python3
"""XGBoost v18 - Optuna Tuning on v1 Proven Features

STRATEGY: Build on what works
- Uses v1's exact feature set (validated: CV 0.48 → LB 0.53)
- Adds Optuna hyperparameter optimization
...
def main():
    print("="*70)
    print("XGBoost v18 - Optuna on v1 Proven Features")
    print("="*70)
    print("Safe incremental improvement strategy")
    print()

    mlflow.set_tracking_uri("http://localhost:5000")
    mlflow.set_experiment("store-sales-xgboost-v1-optuna")
    ...
    # Optuna
    print("Running Optuna (30 trials, ~20 min)...")
    ...
    if study.best_value < 0.4842:
        imp = 0.4842 - study.best_value
        print(f"✓ IMPROVEMENT: -{imp:.4f} ({imp/0.4842*100:.1f}%)")
    else:
    ...
    submission_path = SUBMISSION_DIR / f"xgb_v18_v1_optuna_{cv_str}.csv"
    submission.to_csv(submission_path, index=False)
    print(f"Saved: {submission_path}")

    # MLflow
    with mlflow.start_run(run_name="xgb_v18_v1_optuna"):
        mlflow.log_params(best_params)
        mlflow.log_metric("holdout_cv", study.best_value)
        mlflow.log_metric("vs_v1", study.best_value - 0.4842)
    ...
    print(f"Expected LB: ~{study.best_value + 0.05:.2f} (based on 9.3% gap)")
