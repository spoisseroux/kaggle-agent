#!/usr/bin/env python3
"""LightGBM v37 - Trend Features (Building on v36)

STRATEGY: Add minimal trend features to v36
- v36 baseline: CV 0.3514 (1.6% better than v19)
- Add trend features: 7-day and 30-day linear slopes
- Keep conservative - only 2 new features

Features (17 total - v36 + trends):
- Lags: 3, 7, 14, 21, 28
- Rolling means: 7, 14, 30, 60, 90
- Rolling std: 7
- Trends: 7-day slope, 30-day slope (NEW)
- Temporal: day_of_week, is_weekend, is_holiday
- Promotion: onpromotion

Expected: Small improvement if trends capture momentum patterns
"""
import sys
import numpy as np
import pandas as pd
...
def create_features(df, is_train=True, train_df=None):
    """v36 + trend features (7-day and 30-day slopes)."""
    ...
    if is_train:
        ...
        # NEW: Extended lags
        df["Lag_14"] = df.groupby(["store_nbr", "family"], observed=True)["sales"].shift(14)
        df["Lag_21"] = df.groupby(["store_nbr", "family"], observed=True)["sales"].shift(21)
        df["Lag_28"] = df.groupby(["store_nbr", "family"], observed=True)["sales"].shift(28)
        ...
        # NEW: Trend features (linear slopes)
        df["trend_7"] = (
            df.groupby(["store_nbr", "family"], observed=True)["sales"]
            .transform(lambda x: x.rolling(window=7, min_periods=2).apply(calculate_trend, raw=True))
        )
        df["trend_30"] = (
            df.groupby(["store_nbr", "family"], observed=True)["sales"]
            .transform(lambda x: x.rolling(window=30, min_periods=2).apply(calculate_trend, raw=True))
        )
    else:
        if train_df is not None:
            ...
            # Fill all lags with same approximation (v19 approach)
            df["Lag_3"] = df["lag_mean"].fillna(0)
            df["Lag_7"] = df["lag_mean"].fillna(0)
            df["Lag_14"] = df["lag_mean"].fillna(0)
            df["Lag_21"] = df["lag_mean"].fillna(0)
            df["Lag_28"] = df["lag_mean"].fillna(0)

            for window in [7, 14, 30, 60, 90]:
                df[f"Roll_mean_{window}"] = df["lag_mean"].fillna(0)
            df["Roll_std_7"] = df["lag_std"].fillna(0)

            # Trend features - approximate with 0 for test
            df["trend_7"] = 0.0
            df["trend_30"] = 0.0
...
def main():
    print("="*70)
    print("LightGBM v37 - Trend Features (v36 + 7/30-day slopes)")
    print("="*70)
    print("Building on v36 with minimal trend features")
    print()
    ...
    # Create features (v36 + trend)
    print("Creating v37 features (v36 + trends)...")
    ...
    print(f"Features: {len(feature_cols)} (v36 + 2 trend features)")
    print()
    ...
    print(f"v36 (extended lags): 0.3514")
    print(f"v19 (baseline): 0.3572")

    if holdout_cv < 0.3514:
        improvement = 0.3514 - holdout_cv
        print(f"✓ IMPROVEMENT over v36: -{improvement:.4f} ({improvement/0.3514*100:.1f}%)")
    else:
        decline = holdout_cv - 0.3514
        print(f"✗ Declined vs v36: +{decline:.4f}")
    print()
    ...
    submission_path = SUBMISSION_DIR / f"lgbm_v37_trend_features_{cv_str}.csv"
    submission.to_csv(submission_path, index=False)
    print(f"Saved: {submission_path}")

    # Log to MLflow
    with mlflow.start_run(run_name="lgbm_v37_trend_features"):
        mlflow.log_params(params)
        mlflow.log_metric("holdout_cv", holdout_cv)
        mlflow.log_metric("vs_v36", holdout_cv - 0.3514)
