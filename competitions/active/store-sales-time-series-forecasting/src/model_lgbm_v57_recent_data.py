#!/usr/bin/env python3
"""LightGBM v57 - Recent Data Only (2016+)

HYPOTHESIS: Last 1.5 years more relevant than full 4+ year history
- Older patterns (2013-2014) may dilute recent trends
- Sales patterns evolve over time
- Test set is Aug 2017, closer to 2016-2017 patterns

APPROACH:
- Filter training data to >= 2016-01-01 (1.6 years of data)
- Use v1 proven features (12 features)
- Same LightGBM params as v19

EXPECTED: 2-5% CV improvement over v19's 0.3572

Research: "Use less data: Last 1.5 years performs better than full history"
"""
import sys
import numpy as np
import pandas as pd
...
def main():
    ...
    mlflow.set_experiment("store-sales-lightgbm-v57")

    # Load data
    print("Loading data...")
    train_df, test_df, holidays = load_data()

    # FILTER: Use only recent data (2016+)
    print("\nFiltering to recent data (>= 2016-01-01)...")
    print(f"Full training period: {train_df['date'].min()} to {train_df['date'].max()}")
    train_df = train_df[train_df['date'] >= pd.Timestamp('2016-01-01')]
    print(f"Filtered period: {train_df['date'].min()} to {train_df['date'].max()}")
    print(f"Rows retained: {len(train_df):,}")
    print()
    ...
    print(f"v19 baseline (full data): 0.3572")

    if holdout_cv < 0.3572:
        improvement = 0.3572 - holdout_cv
        print(f"✓ IMPROVEMENT: {improvement/0.3572*100:.1f}% better")
    else:
        decline = holdout_cv - 0.3572
        print(f"✗ WORSE: {decline/0.3572*100:.1f}% worse")
    print()
    ...
    submission_path = SUBMISSION_DIR / f"lgbm_v57_recent_data_{cv_str}.csv"
    submission.to_csv(submission_path, index=False)
    print(f"Saved: {submission_path}")

    # Log to MLflow
    with mlflow.start_run(run_name="lgbm_v57_recent_data"):
        mlflow.log_params(params)
        mlflow.log_params({"data_filter": "2016-01-01_onwards"})
