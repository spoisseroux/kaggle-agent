"""XGBoost v3: Add external data features (oil, transactions, store metadata)

Builds on v2 by adding:
- Store type, cluster from stores.csv
- Oil price and rolling means from oil.csv
- Transaction counts and lags from transactions.csv

Goal: Close CV/LB gap by using all available data.
"""
import numpy as np
import pandas as pd
...
def add_external_data(df, stores, oil_prepared, transactions):
    """Merge external datasets."""
    # Drop existing external columns if they exist (from previous merges)
    for col in ["type", "cluster", "dcoilwtico", "oil_roll_7", "oil_roll_14", "oil_roll_30", "transactions",
                "trans_lag_1", "trans_lag_7", "trans_roll_7"]:
    ...
    if "transactions" in df.columns:
        trans_mean = df.groupby("store_nbr")["transactions"].transform("mean")
        df["transactions"] = df["transactions"].fillna(trans_mean)
        # If still NaN (no data for that store), use global mean
        df["transactions"] = df["transactions"].fillna(transactions["transactions"].mean())
    else:
        # If merge failed completely
        df["transactions"] = transactions["transactions"].mean()
...
def create_lag_features_train(df):
    ...
    # Transaction lags and rolling (group by store only, not family)
    ...
    df["trans_roll_7"] = df.groupby("store_nbr")["transactions"].transform(
        lambda x: x.rolling(window=7, min_periods=1).mean()
    )

    # Re-sort by store/family/date
    df = df.sort_values(["store_nbr", "family", "date"]).reset_index(drop=True)
...
def predict_test_with_lags(model, train_df, test_df, stores, oil_prepared, transactions, holidays, feature_cols, le_type):
    """
    ...
    for test_date in tqdm(test_dates, desc="Predicting test days"):
        ...
        # Add external data
        combined = add_external_data(combined, stores, oil_prepared, transactions)
        ...
        combined["trans_lag_1"] = combined.groupby("store_nbr")["transactions"].shift(1)
        combined["trans_lag_7"] = combined.groupby("store_nbr")["transactions"].shift(7)
        combined["trans_roll_7"] = combined.groupby("store_nbr")["transactions"].transform(
            lambda x: x.rolling(window=7, min_periods=1).mean()
        )
        combined = combined.sort_values(["store_nbr", "family", "date"]).reset_index(drop=True)
...
def main():
    ...
    with mlflow.start_run(run_name="xgb_v3_external_data"):
        # Load data
        train_df, test_df, stores, oil, transactions, holidays = load_data()
        ...
        train_df = add_external_data(train_df, stores, oil_prepared, transactions)
        ...
        feature_cols = [
            ...
            "transactions", "trans_lag_1", "trans_lag_7", "trans_roll_7",
        ]
        ...
        # Log to MLflow
        mlflow.log_param("model", "XGBoost_v3_external_data")
        ...
        submission = predict_test_with_lags(
            model, train_df, test_df, stores, oil_prepared, transactions, holidays,
            feature_cols, le_type
        )

        # Save
        submission_path = SUBMISSION_DIR / f"xgb_v3_external_data_{rmsle:.4f}.csv"
        submission.to_csv(submission_path, index=False)
        print(f"\nSubmission saved: {submission_path}")

        mlflow.log_artifact(str(submission_path))

        print(f"\n✅ XGBoost v3 complete - CV RMSLE: {rmsle:.6f}")
        print(f"Added {len(feature_cols) - 29} new features from external data")
