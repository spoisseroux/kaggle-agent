#!/usr/bin/env python3
"""LightGBM v33 - Hierarchical Models (Per-Family Training)

STRATEGY: Train separate model for each of 33 product families
Different families have different seasonality patterns (AUTOMOTIVE vs GROCERY)

Expected: 3-5% improvement over v19 (LB 0.498 → ~0.47)

Based on top public notebook insights.
"""
import sys
import numpy as np
import pandas as pd
...
def main():
    print("="*70)
    print("LightGBM v33 - Hierarchical Models (Per-Family)")
    print("="*70)
    print("Training separate model for each of 33 product families")
    print()

    mlflow.set_tracking_uri("http://localhost:5000")
    mlflow.set_experiment("store-sales-lightgbm-hierarchical")
    ...
    for family in tqdm(families, desc="Training families"):
        ...
        X_train = train_family.loc[train_mask, feature_cols].fillna(0)
        y_train = train_family.loc[train_mask, "sales"]
        X_val = train_family.loc[val_mask, feature_cols].fillna(0)
        y_val = train_family.loc[val_mask, "sales"]
    ...
    submission_path = SUBMISSION_DIR / f"lgbm_v33_hierarchical_{cv_str}.csv"
    ...
    # Log to MLflow
    with mlflow.start_run(run_name=f"lgbm_v33_hierarchical_{cv_str}"):
        mlflow.log_params({
            "model": "lightgbm_hierarchical",
            "num_families": len(families),
            "features": "v1_proven",
            "validation": "30day_holdout",
            "learning_rate": 0.05,
            "max_depth": 6,
        })
        mlflow.log_metrics({
