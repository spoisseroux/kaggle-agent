#!/usr/bin/env python3
"""LightGBM v23 - v1 Features + Day of Month

STRATEGY: Test different temporal feature
- Start with v19 (best model, CV 0.357, LB 0.498)
- Add day_of_month (1-31) instead of month
- Captures payday patterns, end-of-month shopping

Features (13 total):
- v1 features (12): Lags 3/7, Rolling means 7/14/30/60/90, Roll_std_7,
  day_of_week, is_weekend, is_holiday, onpromotion
- NEW: day_of_month (1-31)
"""
import numpy as np
import pandas as pd
...
def main():
    print("="*70)
    print("LightGBM v23 - v1 Features + Day of Month")
    print("="*70)
    print("Testing day-of-month temporal feature")
    print()
    ...
    # Create features
    print("Creating features (v1 + day_of_month)...")
    ...
    print(f"Features: {len(feature_cols)} (v1 + day_of_month)")
    print()
    ...
    print(f"LightGBM v22 (+month): 0.3584")
    ...
    # Check day_of_month importance
    dom_imp = importance[importance['feature'] == 'day_of_month']
    if not dom_imp.empty:
        dom_rank = (importance['importance'] >= dom_imp.iloc[0]['importance']).sum()
        print(f"\nday_of_month importance: {dom_imp.iloc[0]['importance']:.0f} (rank {dom_rank}/13)")
    print()
    ...
    submission_path = SUBMISSION_DIR / f"lgbm_v23_day_of_month_{cv_str}.csv"
    submission.to_csv(submission_path, index=False)
    print(f"Saved: {submission_path}")

    # Log to MLflow
    with mlflow.start_run(run_name="lgbm_v23_day_of_month"):
        mlflow.log_params(params)
        mlflow.log_metric("holdout_cv", holdout_cv)
        mlflow.log_metric("vs_v19", holdout_cv - 0.3572)
        mlflow.log_metric("vs_v22", holdout_cv - 0.3584)
