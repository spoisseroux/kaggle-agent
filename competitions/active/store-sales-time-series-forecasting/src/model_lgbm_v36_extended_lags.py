#!/usr/bin/env python3
"""LightGBM v36 - Extended Lags (Safe Incremental Improvement)

STRATEGY: Build on proven v19 with minimal changes
- v19 baseline: LB 0.498 (PROVEN to work)
- Add extended lags: 14, 21, 28 days
- Keep everything else identical to v19

Features (15 total - v1 + extended lags):
- Lags: 3, 7, 14, 21, 28 (NEW: 14, 21, 28)
- Rolling means: 7, 14, 30, 60, 90
- Rolling std: 7
- Temporal: day_of_week, is_weekend, is_holiday
- Promotion: onpromotion

Expected: Small improvement, low risk of breaking what works
"""
import sys
import numpy as np
import pandas as pd
...
def main():
    print("="*70)
    print("LightGBM v36 - Extended Lags (v19 + lags 14/21/28)")
    print("="*70)
    print("Safe incremental improvement over proven v19")
    print()
    ...
    submission_path = SUBMISSION_DIR / f"lgbm_v36_extended_lags_{cv_str}.csv"
    submission.to_csv(submission_path, index=False)
    print(f"Saved: {submission_path}")

    # Log to MLflow
    with mlflow.start_run(run_name="lgbm_v36_extended_lags"):
