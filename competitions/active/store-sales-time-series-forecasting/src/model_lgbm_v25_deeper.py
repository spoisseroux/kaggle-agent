#!/usr/bin/env python3
"""LightGBM v25 - Deeper Trees

STRATEGY: Manual conservative hyperparameter adjustment
- Start with v19 (best model, CV 0.357, LB 0.498)
- Change max_depth: 6 → 8
- Test if more capacity helps without overfitting

vs v20 Optuna (which overfit):
- v20: Aggressive multi-param optimization
- v25: Single conservative change
...
def main():
    print("="*70)
    print("LightGBM v25 - Deeper Trees (max_depth=8)")
    print("="*70)
    print("Conservative manual hyperparameter adjustment")
    print()

    mlflow.set_tracking_uri("http://localhost:5000")
    mlflow.set_experiment("store-sales-lightgbm-manual")
    ...
    print("Training LightGBM (max_depth=8)...")
    ...
    print(f"LightGBM v19 (depth=6): 0.3572")
    print(f"LightGBM v20 (Optuna): 0.3516 → LB 0.512 (overfit)")
    ...
    submission_path = SUBMISSION_DIR / f"lgbm_v25_deeper_{cv_str}.csv"
    submission.to_csv(submission_path, index=False)
    print(f"Saved: {submission_path}")

    # Log to MLflow
    with mlflow.start_run(run_name="lgbm_v25_deeper"):
