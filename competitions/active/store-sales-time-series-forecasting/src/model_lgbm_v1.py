"""LightGBM model v1: Better than LinearRegression for time series

Uses same features as baseline_v2 but with gradient boosting model.
Should handle non-linear patterns better.

Target: <0.50 RMSLE (ideally much better)
"""
import numpy as np
import pandas as pd
    ...
    if is_train:
        ...
        if train_df is not None:
            ...
            for window in [7, 14, 30, 60, 90]:
                ...
                df[f"Roll_std_{window}"] = 0
            df["onpromotion_lag1"] = 0
            df["onpromotion_lag7"] = 0
...
def main():
    ...
    mlflow.set_experiment("store-sales-lgbm")

    with mlflow.start_run(run_name="lgbm_v1_full_features"):
        # Load data
        ...
        submission_path = SUBMISSION_DIR / "lgbm_v1_submission.csv"
        submission.to_csv(submission_path, index=False)
        print(f"Submission saved to: {submission_path}")

        mlflow.log_artifact(str(submission_path))

        print(f"\n✅ LightGBM v1 complete - CV RMSLE: {rmsle:.6f}")
