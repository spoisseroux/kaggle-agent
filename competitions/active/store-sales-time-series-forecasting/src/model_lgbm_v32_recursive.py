#!/usr/bin/env python3
"""LightGBM v32 - Recursive Forecasting

BREAKTHROUGH: Top leaderboard uses recursive forecasting, not batch prediction!

TECHNIQUE:
Instead of predicting all 16 test days at once:
1. Predict day 1 using real lag 1-7 from training
2. Add day 1 prediction to data
3. Recalculate features (now lag 1 = day 1 prediction)
4. Predict day 2
5. Repeat for all 16 days

ADVANTAGES:
- Can use powerful short-term lags (1-7 days)
- Features are real, not crude approximations
- Expected: LB 0.376-0.379 (top leaderboard range)

TRADE-OFF:
- Errors accumulate over 16 days
- But much better than v19's "fill all lags with mean"
...
def recursive_forecast(model, train_df, test_df, holidays, feature_cols):
    ...
    """
    print("\n" + "="*70)
    print("RECURSIVE FORECASTING - Predicting 16 days iteratively")
    print("="*70)
    ...
    for day_num, forecast_date in enumerate(tqdm(test_dates, desc="Forecasting")):
        ...
        # Fill any NaN in features with 0 (for early days with insufficient history)
        X_forecast = current_features[feature_cols].fillna(0)
...
def main():
    print("="*70)
    print("LightGBM v32 - Recursive Forecasting")
    print("="*70)
    print("Using v1 features with recursive day-by-day prediction")
    print()

    mlflow.set_tracking_uri("http://localhost:5000")
    mlflow.set_experiment("store-sales-lightgbm-recursive")
    ...
    submission_path = SUBMISSION_DIR / f"lgbm_v32_recursive_{cv_str}.csv"
    submission.to_csv(submission_path, index=False)
    print(f"\nSaved: {submission_path}")

    # MLflow
    with mlflow.start_run(run_name="lgbm_v32_recursive"):
