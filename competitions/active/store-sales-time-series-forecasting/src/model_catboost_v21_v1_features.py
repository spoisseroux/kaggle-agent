#!/usr/bin/env python3
"""CatBoost v21 - v1 Proven Features Test

STRATEGY: Test third model architecture
- Uses v1's exact feature set (validated)
- CatBoost model (after XGBoost and LightGBM)
- Tests: Does model choice continue to matter?

Current best:
- XGBoost v1: LB 0.529
- LightGBM v19: LB 0.498 (BEST)
- CatBoost v21: ?
...
def main():
    print("="*70)
    print("CatBoost v21 - v1 Proven Features")
    print("="*70)
    print("Testing third model architecture")
    print()

    mlflow.set_tracking_uri("http://localhost:5000")
    mlflow.set_experiment("store-sales-catboost-v1")
    ...
    # CatBoost parameters (similar to XGBoost v1)
    params = {
        "loss_function": "RMSE",
        "learning_rate": 0.05,
        "depth": 6,
    ...
    print(f"XGBoost v1: 0.4842 (LB 0.529)")
    print(f"LightGBM v19: 0.3572 (LB 0.498)")

    if holdout_cv < 0.3572:
        improvement = 0.3572 - holdout_cv
        print(f"✓ BETTER than LightGBM: -{improvement:.4f}")
    elif holdout_cv < 0.4842:
        print(f"✓ Better than XGBoost, worse than LightGBM")
    else:
        print(f"✗ Worse than both")
    print()
    ...
    submission_path = SUBMISSION_DIR / f"catboost_v21_v1_features_{cv_str}.csv"
    submission.to_csv(submission_path, index=False)
    print(f"Saved: {submission_path}")

    # Log to MLflow
    with mlflow.start_run(run_name="catboost_v21_v1_features"):
        mlflow.log_params(params)
        mlflow.log_metric("holdout_cv", holdout_cv)
        mlflow.log_metric("vs_xgb_v1", holdout_cv - 0.4842)
        mlflow.log_metric("vs_lgbm_v19", holdout_cv - 0.3572)
    ...
    return {
        ...
        "vs_xgb": holdout_cv - 0.4842,
        "vs_lgbm": holdout_cv - 0.3572,
        "success": True
    }
