#!/usr/bin/env python3
"""Ensemble v81 - Trend Features Only

Based on systematic feature testing showing trend features achieve CV 0.1988 (+49% improvement).

Features added to v67 baseline:
- trend_diff_7: 7-day difference in sales
- trend_7: Simple 7-day trend (current - lag_7)
- trend_14: 14-day trend (current - lag_14) / 2

Simpler than v86 (no ratio features) - less prone to calculation errors.
"""
import sys
import numpy as np
import pandas as pd
...
def create_features(df):
    """v67 baseline + v81 trend features"""
...
def main():
    print("="*70)
    print("Ensemble v81 - Trend Features Only")
    print("="*70)
    print("v67 baseline + trend features")
    print("Total: 15 features (12 baseline + 3 trend)")
    print()
    ...
    # Test features - CRITICAL: Keep track of original test IDs
    ...
    # CRITICAL FIX: Filter by original test IDs only (not just notna)
    ...
    # Verify we have correct number of test predictions
    assert len(test_preds) == len(original_test_df), f"Wrong number of predictions: {len(test_preds)} vs {len(original_test_df)}"
    ...
    print(f"  Rows: {len(submission)} (expected: {len(original_test_df)})")
    print(f"  ID range: {submission['id'].min()} - {submission['id'].max()}")
    print(f"  Unique IDs: {submission['id'].nunique()}")
    print()

    cv_str = f"{cv:.4f}".replace(".", "")
    submission_path = SUBMISSION_DIR / f"ensemble_v81_trend_{cv_str}.csv"
